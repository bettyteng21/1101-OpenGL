...
char move_dir = 'q';
float view_x = 0.0, view_z = 0.0, fire_z=0.0, logo_z=7.0, explode_rad=1.0, jump_y=0.0; 
int year = 0, day = 0, logo_angle=0, jump_time=0;
...
void glDrawArrays_logo(void) {
	...
	if (logo_angle < 60) {
		...
		glColor3f(0.50196f, 1.0f, 0.0f);
	}
	else if (logo_angle >= 180 && logo_angle < 240) {
		glColor3f(0.0f, 0.50196f, 1.0f);
	}
	else if (logo_angle >= 240 && logo_angle < 300) {
		glColor3f(0.25098f, 0.0f, 1.0f);
	}
...
void fire_solar_system(void) {
	...
	if (logo_z + fire_z <= 0.4) { //if hit (logo和太陽系的相對距離夠小)
		//logo的x，最左是-2.62，最右是+2.64 (以0為基準)
		//且太陽系的x是固定-1.5

		if ((view_x - 2.62) > -0.62 || (view_x + 2.64) < -1.05) { //miss
			printf("miss\n");
...
void auto_rotate(void) {
	...
	if (fire == false && is_hit == true) {  //命中後，爆炸的火花直徑會變大
		explode_rad += 0.5;
	}
...
void display(void)	
{
	...
	glPushMatrix();		
		...
		switch (move_dir) {
			...
			case 'd':
				...
				break;
			case 'z':
				fire = true; is_hit = false; fire_z = 0.0; explode_rad = 1.0; recover = false;
				break;
			case 'x':
		...
		if (is_hit == true && explode_rad > 2 && recover==false) { //爆炸後logo消失
		}
		else { //其餘時間都要畫logo
	...
	glPushMatrix();
		...
		if(fire==true) fire_solar_system(); //按下發射鍵，尚未抵達
			...
			if (explode_rad <= 2 && explode_rad > 1) { //hit且正在做explode動畫
				glPushMatrix();
					GLfloat mat_ambient_red[] = { 1.0, 0.0, 0.0, 1.0 };
					glMaterialfv(GL_FRONT, GL_AMBIENT, mat_ambient_red);
			...
			else draw_solar_system_left(); //miss or 尚未發射 or 爆炸已結束redraw
		}
	glPopMatrix();
