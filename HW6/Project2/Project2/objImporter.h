...
Model loadModel(char *fileName){
  ...
  while(fgets (mystring , 100 , pFile) != NULL ){
	...
	//total of vertex
	objModel.totV=i;
	objModel.v = (vertex *)malloc(i * sizeof(vertex));
	rewind(pFile);
  i=0;
  ...
  //count the faces number
	...
	//total of faces
	objModel.totF=i;
	objModel.f = (faces*)malloc(i * sizeof(vertex));
	rewind(pFile);
	i=0;
  ...
  while(fgets (mystring , 100 , pFile) != NULL ){
  	for (i=0;i<strlen(mystring);i++) if(mystring[i]==' ') space++;
        ...
        if(space==3){
        	objModel.f[j].Faces = (float *)malloc (3 * sizeof(float));
				  objModel.f[j].vNumber=3;
        }
        if(space==4){
        	objModel.f[j].Faces = (float *)malloc (4 * sizeof(float));
